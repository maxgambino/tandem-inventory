...
r.get('/items', async (req, res) => {
  ...
  try {
    ...
    const qtyIndex = new Map<string, Map<string, number>>();

    const addQty = (pid: string, lid: string | null | undefined, delta: number) => {
      if (!lid) return;
      if (!qtyIndex.has(pid)) qtyIndex.set(pid, new Map());
    ...
    for (const m of movements) {
      ...
      switch (m.type) {
        ...
        case MovementType.ADJUSTMENT:
          ...
          if (m.toLocationId) addQty(m.productId, m.toLocationId, +q);
          else if (m.fromLocationId) addQty(m.productId, m.fromLocationId, -q);
          break;
...
r.post('/movements', async (req, res) => {
  try {
    ...
    if (type === 'IN' && !toLocationId) return res.status(400).json({ error: 'toLocationId requis pour IN' });
    if (type === 'OUT' && !fromLocationId) return res.status(400).json({ error: 'fromLocationId requis pour OUT' });
